...
public class TripleSDESCracker {
	...
	public static byte[][] crack(String encrypted){
		byte[] key1 = {};
		byte[] key2 = {};
		byte[] first = SDEScracker.stringToByteArray("0000000000");
		byte[] last = SDEScracker.stringToByteArray("1111111111");
		
		byte[] encryptedBytes = SDEScracker.stringToByteArray(encrypted);
		
		int upperlimit = SDEScracker.getDecimal(last);
		...
		for(int i = 0; i < upperlimit; i++){
			...
			key1 = SDEScracker.getByte(i, 10);
			
			for(int j = 0; j < upperlimit; j++){
				
				key2 = SDEScracker.getByte(j, 10);
	...
	public static byte[] longDecrypt(byte[] key1, byte[] key2, byte[] cipher){
		...
		for(byte[] group: arrays){
			text = SDEScracker.concatenateByteArrays(text, TripleSDES.Decrypt(key1, key2, group));
		}
